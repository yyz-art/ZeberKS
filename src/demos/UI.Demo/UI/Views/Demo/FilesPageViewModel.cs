...
public partial class FilesPageViewModel : ObservableObject
{
			...
			Columns =
			{
				...
				new HierarchicalExpanderColumn<FileNodeViewModel>(
					new TemplateColumn<FileNodeViewModel>(
						...
						{
							CompareAscending = FileNodeViewModel.SortAscending<string>(vm => vm.Name),
							CompareDescending = FileNodeViewModel.SortDescending<string>(vm => vm.Name),
				...
				new TextColumn<FileNodeViewModel, long?>(
					...
					{
						CompareAscending = FileNodeViewModel.SortAscending<long?>(x => x.Size),
						CompareDescending = FileNodeViewModel.SortDescending<long?>(x => x.Size),
					}),
				new TextColumn<FileNodeViewModel, DateTimeOffset?>(
					...
					options: new TextColumnOptions<FileNodeViewModel>
					{
						CompareAscending = FileNodeViewModel.SortAscending<DateTimeOffset?>(x => x.Modified),
						CompareDescending = FileNodeViewModel.SortDescending<DateTimeOffset?>(x => x.Modified),
					}),
			}
		};
	...
	private void SelectionChanged(object? sender, TreeSelectionModelSelectionChangedEventArgs<FileNodeViewModel> e)
	{
		...
		this.SetField(ref _selectedPath, selectedPath, null, nameof(SelectedPath));
...
public partial class FileNodeViewModel : ObservableObject, IEditableObject
{
	...
	{
		Path = path;
		Name = (isRoot ? path : System.IO.Path.GetFileName((string?)Path))!;
		IsExpanded = isRoot;
	...
	void IEditableObject.CancelEdit() => Name = _undoName!;
