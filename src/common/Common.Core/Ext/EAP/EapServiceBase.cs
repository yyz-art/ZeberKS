...
public class EapServiceBase : IPassiveAcceptor<Socket>, INamedObject
{
	...
	public required TcpServerSocket ServerSocket { get; init; }
	...
	private async Task ClientMain(NetworkSocket socket)
	{
		...
		var clientBuffer = new byte[102400];
		try
		{
			while (socket.IsOpen)
			{
				...
				var request = JsonNode.Parse(clientBuffer.AsSpan(0, readLength)) as JsonObject;
				if (request is null)
				{
				...
				var stream = request["Stream"]?.AsValue().GetValue<int>();
				if (stream is null)
				{
					Logger.Error($"Client Receive Error: json.Stream is null");
					break;
				}


				var function = request["Function"]?.AsValue().GetValue<int>();
				if (function is null)
				{
					Logger.Error($"Client Receive Error: json.Function is null");
					break;
				}

				Logger.Info("CLIENT '{}' REQ: S{stream}F{function}", socketKey, stream.Value, function.Value);
				var handlerKey = (stream.Value * 1000) + function.Value;
