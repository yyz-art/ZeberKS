...
public class MesServiceBase<T> : INamedObject where T : CommonAppConfig
{
    ...
    public Result<Message1Response> SendMessage1(string operatorID, string sn)
    {
       lock (_syncLock)
       {
           try 
           {
               ...
               var requestLog = $"Line: {request.Line}, Op: {request.OperatorID}, Type: {request.MessageType}, Result: {request.Result}, SN: {request.SN}, Station: {request.StationName}";
    ...
    public Result<Message7Response> SendMessage7(string operatorID, string sn, string customStation = null, string customLine = null)
    {
        lock (_syncLock)
        {
            try
            {
                ...
                var requestLog = $"Line: {request.Line}, Op: {request.OperatorID}, Type: {request.MessageType}, SN: {request.SN}, Station: {request.StationName} | 查询变量: MO_NUMBER, MODEL_NAME";
       ...
       string operatorID, string sn, SifsPayloadBuilder? payload, string productResult = "OK")
    {
       lock (_syncLock)
       {
           try
           {
               ...
               var requestLog = $"Line: {request.Line}, Op: {request.OperatorID}, Type: {request.MessageType}, Result: {request.Result}, SN: {request.SN}, Station: {request.StationName} | {payloadLog}";
    ...
    public Result<string> SendRawAsciiMessage2WithScrewData(string operatorID, string sn, IReadOnlyList<(double Torque, double Turns)> screwData)
    {
        lock (_syncLock)
        {
            try
            {
                ...
                string requestAscii = $"{cleanStation},{cleanSn},2,{cleanOp},{cleanLine},,OK,,,{string.Join(" ", payloadParts)}";
    ...
    private static void WriteMesLog(string direction, string messageType, string content)
    {
        try
        {
            ...
            var text = $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}] [{direction}] [{messageType}]{Environment.NewLine}{normalizedContent}{Environment.NewLine}{Environment.NewLine}";
