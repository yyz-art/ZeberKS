...
public partial class CoreService : CoreServiceBase
{
	...
	public async Task<MesRecipePrepareState> PrepareRecipeByMesModelNameAsync(string modelName)
	{
		...
		try
		{
			...
			var plcAckState = await RequestPlcRecipeWriteCompleteAckAsync();
	...
	private async Task<MesRecipePrepareState> RequestPlcRecipeWriteCompleteAckAsync()
	{
		var resetResult = await ResetRecipeCompleteHandshakeAsync("配方切换");
		...
		while (DateTime.Now < timeoutAt)
		{
			...
			{
				Logger.Info("【配方切换】收到 PLC 响应=1，复位 PC配方写入完成信号(21230) 和 PLC读取配方ID切换成功响应(22414)。");
				var clearResult = await ResetRecipeCompleteHandshakeAsync("配方切换");
				if (clearResult.IsError())
				{
					Logger.Error("【配方切换】成功响应后复位配方完成握手信号失败: {msg}", clearResult.Message);
					return MesRecipePrepareState.Failed;
				}
