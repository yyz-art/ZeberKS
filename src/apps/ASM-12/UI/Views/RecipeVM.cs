...
public partial class RecipeVM : UiVM<RecipeView>
{
	...
	public async Task @SelectManualVisionImagePath()
	{
		...
		AppConfig.ManualVisionImagePath = folders[0].Path.LocalPath;
		await SaveManualVisionConfigAsync();
		ShowToast("图片路径已选择", UiMessageType.Success);
	}

	public async Task @PackManualVisionImages()
	{
		var manualSn = AppConfig.ManualVisionSn?.Trim() ?? "";
		var imageFolder = AppConfig.ManualVisionImagePath?.Trim() ?? "";

		if (string.IsNullOrWhiteSpace(manualSn))
		{
			await ShowMessageBox("手动SN不能为空。", "视觉手动打包", MessageBoxIcon.Warning);
			return;
		}

		if (string.IsNullOrWhiteSpace(imageFolder) || Directory.Exists(imageFolder) == false)
		{
			await ShowMessageBox("图片路径不存在，请重新选择。", "视觉手动打包", MessageBoxIcon.Warning);
			return;
		}

		var imageFiles = Directory.EnumerateFiles(imageFolder, "*.*", SearchOption.AllDirectories)
			.Where(IsImageFile)
			.ToArray();

		if (imageFiles.Length == 0)
		{
			await ShowMessageBox("当前路径下没有找到图片文件。", "视觉手动打包", MessageBoxIcon.Warning);
			return;
		}

		try
		{
			var outputDir = @"D:\MesZip";
			Directory.CreateDirectory(outputDir);
			var safeSn = SanitizeFileName(manualSn);
			var zipPath = Path.Combine(outputDir, $"{Guid.NewGuid():N}_{safeSn}.zip");

			using (var archive = ZipFile.Open(zipPath, ZipArchiveMode.Create))
			{
				foreach (var imageFile in imageFiles)
				{
					var entryName = Path.GetRelativePath(imageFolder, imageFile).Replace('\\', '/');
					archive.CreateEntryFromFile(imageFile, entryName, CompressionLevel.Optimal);
				}
			}

			AppConfig.ManualVisionSn = manualSn;
			AppConfig.ManualVisionImagePath = imageFolder;
			await SaveManualVisionConfigAsync();
			ShowNotification($"打包完成：{zipPath}", UiMessageType.Success);
		}
		catch (Exception ex)
		{
			await ShowMessageBox($"打包失败：{ex.Message}", "视觉手动打包", MessageBoxIcon.Error);
		}
	}

	private async Task SaveManualVisionConfigAsync()
	{
		var currentConfigContext = AppConfig.AccessConfigContext() as JsonConfigContext;
		if (currentConfigContext?.SavePath == null)
			return;

		var result = await ConfigManager.SaveConfigAsync(AppConfig, currentConfigContext.SavePath);
		if (result.IsError())
			ShowNotification($"保存视觉配置失败! {result}", UiMessageType.Error);
	}
