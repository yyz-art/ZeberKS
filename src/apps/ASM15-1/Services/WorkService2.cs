	...
	protected override Task Main(CancellationToken ctk)
	{
		...
		while (ctk.IsCancellationRequested == false)
		{
			...
			if (Plc.Read is { 工位2数据上报请求: 1, 工位2数据上报响应: 0 })
			{
				...
				DataBuilder.Append($"[VR]GUID='{uuid}'\"");
				var mes7 = $"ASM_QUERY,{ScanSnCode},7,,SMD,,OK,GET_PN=??? ZEBRA_OTG=??? SN_NEW_GROUP";
				var mesMsg7Result = Mes.SendAndReadString(mes7);
				var topic = GenerateTopic(mesMsg7Result.Value!, AppConfig.StationName);
				Task.Run(()=>CreateImagePackageZip("D:/Vision Files",["data/NLog.config"],ScanSnCode, topic,uuid));
#elif ASM15_1
				for (var i = 0; i < ScrewCount; i++)
				{
					var screwNo = i + 1; // 8.9 15.5
