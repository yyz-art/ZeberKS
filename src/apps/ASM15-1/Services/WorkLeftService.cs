...
public partial class WorkLeftService : WorkServiceBase
{
	...
	private Result<string> ScanCodeOnce(char[] charBuffer)
	{
		...
		Plc.Write.扫码枪1触发 = 0;
		Plc.Write.WritePoint(PlcStructInfo.扫码枪1触发).Unwarp("reset left scanner trigger failed!");
		Logger.Info("扫码枪1触发命令发送成功，已复位 扫码枪1触发=0。");
		...
		if (false == _buffer.StartsWith(StartScanResponseBytes))
		{
			...
			return Result.Err<string>("扫码枪1响应格式错误");
		}
		...
		if (false == Encoding.UTF8.TryGetChars(codeBytes, charBuffer, out var codeLength))
		{
			...
			return Result.Err<string>("扫码枪1条码解析失败");
		}

		var code = new string(charBuffer, 0, codeLength).Trim();
		if (string.IsNullOrWhiteSpace(code) || code.Length < 20)
		{
			Logger.Error("扫码枪1未扫到有效二维码或二维码长度不足20位，当前码='{code}'，长度={length}，工位1不允许生产。", code, code.Length);
			return Result.Err<string>("扫码枪1未扫到有效二维码或二维码长度不足20位");
		}
		...
		while (ctk.IsCancellationRequested == false)
		{
			...
			if (Plc.Read.扫码枪1触发 == 1 && Plc.Read.扫码枪1触发结果 == 0)
			{
				...
				try
				{
					...
					if (_waitingSecondScan)
					{
						...
						{
							Logger.Warn("【MES跳过】工位1第二次扫码完成，跳过 MSG3，写允许生产=1。SN1={sn1}, SN2={sn2}", _sn1, _sn2);
					...
					if (Core.MesSkipEnabled)
					{
						Logger.Warn("【MES跳过】工位1第一次扫码完成，跳过 MSG7/MSG1，仅写扫码枪1触发结果=1，等待第二次扫码。SN1={sn1}", _sn1);
					...
					Logger.Info("【第三步】收到 MES 回复前三位包含 OK，仅写扫码枪1触发结果=1，等待到位后第二次扫码。SN1={sn1}", _sn1);
					_waitingSecondScan = true;
					goto SendCodeOK;
				}
				catch (Exception ex)
			...
			if (Plc.Read.工位1数据上报请求 == 1 && Plc.Read.工位1数据上报响应 == 0)
			{
				try
				{
					...
					{
						Logger.Error("【过站】收到 工位1数据上报请求(22015)=1，但16颗螺丝数据未收齐，缺少螺丝: {missingScrews}。", missingScrews);
						ResetWorkLeftUploadRequest();
						_pendingUploadResponse = 2;
						goto WritePendingResponse;
					}
					...
					if (Core.MesSkipEnabled)
					{
						Logger.Warn("【MES跳过】收到 工位1数据上报请求(22015)=1，16颗螺丝数据已收齐，跳过 MSG2，直接复位请求并响应 OK。");
						ResetWorkLeftUploadRequest();
						_pendingUploadResponse = 1;
						goto WritePendingResponse;
					}

					Logger.Info("【过站】收到 工位1数据上报请求(22015)=1，先复位请求信号，再发送带16颗螺丝数据的 MES MSG2。");
					ResetWorkLeftUploadRequest();
	...
	private void ResetWorkLeftUploadRequest()
	{
		Plc.Write.工位1数据上报请求 = 0;
		Plc.Write.WritePoint(PlcStructInfo.工位1数据上报请求).Unwarp("reset work left upload request failed!");
	}
	...
	{
		var screwNo = Plc.Read.工位1螺丝拧紧完成信号;
		if (screwNo == 0)
			return false;
		...
		try
		{
			...
			{
				Logger.Warn("【螺丝采集】收到工位1螺丝拧紧完成信号(22075)={screwNo}，但当前扫码流程未完成，忽略并复位。", screwNo);
				return ResetLeftScrewFinishedSignal();
			}

			if (screwNo is < 1 or > 16)
			{
				Logger.Warn("【螺丝采集】收到工位1螺丝拧紧完成信号(22075)={screwNo}，超出1-16范围，忽略并复位。", screwNo);
				return ResetLeftScrewFinishedSignal();
			}

			var screwService = App.Current.ScrewService1;
			if (screwService is null)
			{
				Logger.Error("【螺丝采集】工位1螺丝服务未启动，无法保存第{screwNo}颗螺丝数据。", screwNo);
				return ResetLeftScrewFinishedSignal();
			}

			if (IsScrewServiceFresh(screwService) == false)
			{
				Logger.Error("【螺丝采集】工位1螺丝机未连接或实时数据已过期，无法保存第{screwNo}颗螺丝数据。连接状态={isConnected}，最后成功读取时间={lastReadAt:yyyy-MM-dd HH:mm:ss.fff}",
			...
			Logger.Info("【螺丝采集】工位1第{screwNo}颗螺丝保存完成，扭力={torque}，圈数={turns}，SN1={sn1}，SN2={sn2}。",
				screwNo, snapshot.MaxTorque, snapshot.Turns, snapshot.Sn1, snapshot.Sn2);
			return ResetLeftScrewFinishedSignal();
		}
		catch (Exception ex)
		{
			Logger.Error(ex, "【螺丝采集】工位1处理螺丝完成信号失败。");
			return false;
		}
	}

	private bool ResetLeftScrewFinishedSignal()
	{
		Plc.Write.工位1螺丝拧紧完成信号 = 0;
		Plc.Write.WritePoint(WorkScrewSignalPoints.LeftScrewFinishedSignal).Unwarp("reset work left screw finished signal failed!");
		return true;
	}
