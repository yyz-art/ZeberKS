...
public partial class RecipeVM : UiVM<RecipeView>
{
	...
	{
		foreach (var groupInfo in TempRecipe.GetStructInfo().Members.Values.OfType<IBinaryPointGroupInfo>())
		{
			...
			var readResult = await TempRecipe.ReadPointGroupAsync(groupInfo, reader);
		...
		ShowToast("start read all points");
		foreach (var propertyInstance in FilteredPropertyInstances)
		{
			if (propertyInstance.Flag == false)
				continue;
