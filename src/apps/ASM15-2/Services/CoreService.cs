...
public partial class CoreService : CoreServiceBase
{
	...
	private static readonly IBinaryPointInfo PlcRecipeIdSwitchSuccessResponsePoint = new BinaryPointInfo
	{
		Offset = 22414,
		Name = nameof(PlcStruct.PLC读取配方ID切换成功响应),
		Type = typeof(short),
		ByteLength = 2,
	...
	private static readonly IBinaryPointInfo PcRecipeWriteCompletePoint = new BinaryPointInfo
	{
		Offset = 21230,
		Name = nameof(PlcStruct.PC配方写入完成信号),
		Type = typeof(short),
		ByteLength = 2,
		ByteFormat = ByteFormat.CDAB,
		RawType = typeof(short),
	...
	private async Task<MesRecipePrepareState> RequestPlcRecipeWriteCompleteAckAsync(string logPrefix = "配方切换")
	{
		...
		{
			await Task.Delay(200);
			var resetReadResult = await Plc.Read.ReadPointAsync(PlcRecipeIdSwitchSuccessResponsePoint);
		...
		var writeResult = await Plc.Write.WritePointAsync(PcRecipeWriteCompletePoint);
		...
		{
			await Task.Delay(200);
			var readResult = await Plc.Read.ReadPointAsync(PlcRecipeIdSwitchSuccessResponsePoint);
	...
	private async Task<Result> ResetRecipeCompleteHandshakeAsync(string logPrefix)
	{
		...
		var clearRequestResult = await Plc.Write.WritePointAsync(PcRecipeWriteCompletePoint);
		...
		var readResponseResult = await Plc.Read.ReadPointAsync(PlcRecipeIdSwitchSuccessResponsePoint);
		...
		var clearResponseResult = await Plc.Write.WritePointAsync(PlcRecipeIdSwitchSuccessResponsePoint);
