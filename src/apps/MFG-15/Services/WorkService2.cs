...
public partial class WorkService2 : WorkServiceBase
{
	...
	public override string ServiceName => "WP-2";
...
#if ASM15_1
			if (Plc.Read is { 扫码枪2触发: 2, 扫码枪2触发结果: 0 }) // SCAN KEY PART 
#else
			if (Plc.Read is { 扫码枪2触发: 1, 扫码枪2触发结果: 0 }) // SCAN SN 
#endif
			{
				...
				Plc.Write.扫码枪2触发 = 0;
				Logger.Info(
					$"[MES IN-STA] [DONE] SCAN=({Plc.Write.扫码枪2触发结果},'{ScanSnCode}') EN={Plc.Write.工位2允许生产}");
				...
				Plc.Write.工位2生产序号 = (short)DayProductionId;
				Plc.Write.WritePoint(nameof(PlcStruct.工位2生产序号));
				Plc.Plc.Write($"{PlcStructInfo.扫码枪2扫码内容.Offset}", ScanSnCode, 80);
				Plc.Write.TryWritePoint(nameof(PlcStruct.扫码枪2触发结果), this, static ctx =>
				{
					ctx.Context.Logger.Error(
						$"[MES IN-STA] [ERROR] write plc code scan signal failed by plc connection error! {ctx.Result.Message}");
